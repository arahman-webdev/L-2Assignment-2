...
SELECT name, COUNT(name) sighting_id
FROM sightings
JOIN rangers ON sightings.ranger_id = rangers.ranger_id
GROUP BY name;

SELECT name sighting_id
FROM sightings
JOIN rangers ON sightings.ranger_id = rangers.ranger_id
GROUP BY name;
...
WHERE DATE (discovery_date) < '1825-01-01'
...
CASE 
    ...
    WHEN date_part('hour', sighting_time) BETWEEN 12 AND 5 THEN  'Afternoon'
    ELSE  'Unknown'
END AS time_of_day
FROM sightings;
...
DELETE FROM rangers
WHERE NOT EXISTS(SELECT 1 FROM sightings WHERE sightings.ranger_id = rangers.ranger_id)
